...
class CodeSubmission(Base):
    ...
    chats =  relationship("Message",  back_populates="my_submission", cascade="all, delete-orphan")
...
class Message(Base):
    ...
    id = Column(INTEGER, primary_key=True)
    submission_id = Column(INTEGER, ForeignKey("Codes.id", ondelete="CASCADE"), nullable=False)
    question = Column(String, nullable=False)  # "user" or "assistant"
    answer = Column(String, nullable=False)
    created_at = Column(TIMESTAMP(timezone=True), nullable=False, server_default=text('now()'))
    my_submission = relationship("CodeSubmission", back_populates="chats")
